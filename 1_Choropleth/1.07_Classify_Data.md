## Tutorial 1.7: Classifying Attribute Data

**Summary**

**Tutorial 1.7** demonstrates how to ***classify*** data to make an appropriate choropleth map, which uses a colour ramp to display differences in data *(Figure 1.7-1)*. 
...
![](1.07_Classify_Data_images/image_0.png)

###### Figure 1.7-1: Choropleth map based on classified data.
...
Move the *Layer Properties* window to see that the dark black borders on the map have changed to light grey, reducing their presence on the visual hierarchy (*Figure 1.7-2*, see **Section 2.13**).

![](1.07_Classify_Data_images/image_1.png)

###### Figure 1.7-2: Visual hierarchy, book Figure 2.13-2.
...
Select the colour ramp (*Figure 1.7-3*, see **Section 2.10**). Click the arrow on the right side of the *Colour ramp*. Then click *Create New Colour Ramp…* and change the *Colour ramp type* to *Catalog: ColorBrewer*. Click *OK*. 

![](1.07_Classify_Data_images/image_2.png)

###### Figure 1.7-3: Dimensions of colour, book Figure 2.10-1.
...
Click the *Histogram* tab and then click *Load Values* to see how the data is distributed *(Figure 1.7-4)*. Viewing the ***histogram*** is a good idea to help determine how to classify your data.

> ***Histogram***: a univariate diagram that bins numerical data into mutually exclusive and exhaustive classes (see **Section 3.11**)

![](1.07_Classify_Data_images/image_3.png)

###### Figure 1.7-4: Histogram of the attribute data.
...
Some countries are ***missing data*** but should still appear on the map (*Figure 1.7-5*).

> ***Missing data***: the absence of an attribute value for a particular location and year (see **Section 2.15**)

![](1.07_Classify_Data_images/image_4.png)

###### Figure 1.7-5: Representing missing data, book Figure 2.15-1.
...
Now you have classified the data and applied an appropriate colour scheme. Save and proceed to [**Tutorial 1.8**: Visual Accenting](/1_Choropleth/1.08_Visual_Accenting.md).
