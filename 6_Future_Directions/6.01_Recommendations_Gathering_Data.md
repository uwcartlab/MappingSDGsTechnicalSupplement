## Tutorial 6.1: Recommendations on Gathering Data
...
These tutorials provide data pre-downloaded and cleaned from the United Nations Global SDG Indicators Database and join these data to the UN shapefile or Natural Earth open-source shapefile. A ***join*** is a way to associate two different datasets temporarily and is common in cartography for the purposes of spatial analysis and mapping. Joins allow you to associate attribute data in a table format, like a CSV file (comma separated values), with a geospatial file, like a SHP (shapefile file, a common format developed by Esri manipulatable in most mapping software). [**Tutorial 1.3**](/1_Choropleth/1.03_Download_Data.md) demonstrates how to download these datasets and [**Tutorial 1.6**](/1_Choropleth/1.06_Add_Data.md) demonstrates how to make joins.
...
Often, it can be easier to clean data in spreadsheet software like Google Sheets or Libre Office, rather than QGIS or other mapping software. For example, if one data source has County tacked onto the end of each name, while the other does not, you could use the *Find and Replace* feature in Excel to replace the word County with empty space.
